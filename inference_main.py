...
def main():
    ...
    diffusion_model_path = args.diffusion_model_name
    diffusion_config_path = args.diffusion_config_name
    k_step = args.k_step
    ...
    if cluster_infer_ratio != 0:
        if args.cluster_model_name == "":
            if args.feature_retrieval:  # 若指定了占比但没有指定模型路径，则按是否使用特征检索分配默认的模型路径
                args.cluster_model_name = "logs/44k/feature_and_index.pkl"
            else:
                args.cluster_model_name = "logs/44k/kmeans_10000.pt"
    else:  # 若未指定占比，则无论是否指定模型路径，都将其置空以避免之后的模型加载
        args.cluster_model_name = ""

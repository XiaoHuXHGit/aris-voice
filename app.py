...
def load_model_func(ckpt_name,cluster_name,config_name,enhance,diff_model_name,diff_config_name,only_diffusion,use_spk_mix,using_device,method,speedup,cl_num):
    ...
    if diff_model_name != "no_diff":
        ...
        if not only_diffusion:
            if _content['data'].get('encoder_out_channels') != config["model"].get('ssl_dim'):
                return "扩散模型维度与主模型不匹配，请确保两个模型使用的是同一个编码器", gr.Dropdown(choices=[], value=""), 0, None
    ...
    #读取模型各维度并比对，还有小可爱无视提示硬要加载底模的就返回个未初始张量
        ...
        if emb_dim > config["model"]["n_speakers"]:
            return "模型说话人数量与emb维度不匹配", gr.Dropdown(choices=[], value=""), 0, None
        if model_dim != config["model"]["ssl_dim"]: 
            return "配置文件与模型不匹配", gr.Dropdown(choices=[], value=""), 0, None
    ...
    return (
        output_msg, 
        gr.Dropdown(choices=spk_list, value=spk_choice),
        clip, 
        gr.Slider(value=100 if k_step_max>100 else k_step_max, minimum=speedup, maximum=k_step_max)
    )
...
def source_change(use_microphone):
    if use_microphone:
        return vc_input3.update(sources=["microphone", "upload"])
    else:
        return vc_input3.update(sources=["upload", "microphone"])
...
#read default params
sovits_params, diff_params, second_dir_enable = get_default_settings()  # , sami_settings
ckpt_read_dir = second_dir if second_dir_enable else workdir
config_read_dir = second_dir if second_dir_enable else config_dir
diff_read_dir = diff_second_dir if second_dir_enable else diff_workdir
current_mode = get_current_mode()
...
with app:
    gr.Markdown(value="""
        # So-VITS-SVC 4.2 unrecognizable by 小狐
	这玩意，真是磨人的小妖精啊。。。快整死我了
        """)
    with gr.Tabs():
        with gr.TabItem("推理") as inference_tab:
            ...
            with gr.Tabs():
                with gr.TabItem("单个音频上传"):
                    vc_input3 = gr.Audio(label="单个音频上传", type="filepath", sources=["upload", "microphone"])
                ...
                with gr.TabItem("文字转语音"):
                    ...
                    with gr.Row():
                        ...
                        tts_lang = gr.Dropdown(label = "选择语言，Auto为根据输入文字自动识别", choices=SUPPORTED_LANGUAGES, value = "Auto", allow_custom_value=True)
                    with gr.Row():
        ...
        with gr.TabItem("训练"):
            ...
            with gr.Row():
                ...
                """)
            with gr.Row():
                branch_selection = gr.Dropdown(label="选择训练使用的编码器", choices=encoder_list, value="vec768l12", interactive=True, allow_custom_value=True)
                f0_predictor_selection = gr.Dropdown(label="选择训练使用的f0预测器", choices=f0_options, value="rmvpe", interactive=True, allow_custom_value=True)
            with gr.Row():
    ...
    with gr.Tabs():
        ...
        app.queue(max_size=2044).launch(server_name="127.0.0.1",inbrowser=True,quiet=True)
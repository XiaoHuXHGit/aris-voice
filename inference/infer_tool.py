...
class Svc(object):
    ...
    def infer(self, speaker, tran, raw_path,
        ...
        wav, sr = torchaudio.load(raw_path, backend="soundfile")
    ...
    def slice_inference(self,
        ...
        # else:
        #     audio_stream = io.BytesIO(raw_audio_path)
        #     audio, sr = librosa.load(audio_stream, sr=None)
        #     chunks = slicer.cut_byte(audio, sr, db_thresh=slice_db)
        ...
        global_frame = 0
        audio = []
        print(f'启动第二阶段SING VOICE CONVERSION推演')
        for (slice_tag, data) in audio_data:
            # 这是切分并打印输出的分支，没啥用，注释掉
            ...
            for k,dat in enumerate(datas):
                ...
                # yield np.array(list(_audio))
        print('人工智能推演SING VOICE CONVERSION完成')
